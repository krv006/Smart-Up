...
def fetch_balance_chunks(cursor, filial_warehouse_list, product_conditions, user_begin_date: datetime, user_end_date: datetime):
    """
        ...
        for cond in product_conditions:
            ...
            for start, finish in daterange(effective_begin, effective_end, step_days=30):
                ...
                try:
                    ...
                    print(f"⚠️ API xatosi | {scope_key} | cond:{cond} | {start.strftime(DATE_FORMAT)} - {finish.strftime(DATE_FORMAT)} | {e}")
    ...
    print(f"Σ API items: {total_items} | fact_rows:{len(fact_rows)} | group_rows:{len(group_rows)} | condition_rows:{len(condition_rows)}")
...
def main():
    ...
    fact_rows, group_rows, condition_rows = fetch_balance_chunks(cursor, filial_warehouse_list, product_conditions, begin_date, end_date)
...
""")
    ...
    print(f"💾 Yuklash yakunlandi | Fact yozuvlar: {len(fact_rows)} | Group yozuvlar: {len(group_rows)} | Condition yozuvlar: {len(condition_rows)}")
