...
BEGIN
  ...
  (
      scope_key         nvarchar(200) NOT NULL PRIMARY KEY, -- masalan: "filial=1|warehouse=65478"
...
def make_scope_key(filial_id, warehouse_id) -> str:
...
# ====== API → ROWS (INCREMENTAL) ======
def fetch_balance_chunks(cursor, filial_warehouse_list, user_begin_date: datetime, user_end_date: datetime):
    """
    Har bir (filial_id, warehouse_id) scope bo‘yicha LoadState’ni o‘qiydi:
      effective_begin = max(user_begin_date, (state_date - buffer))
      effective_end   = user_end_date
    Qaytadi: fact_rows, group_rows
    """
    session = requests.Session()
    fact_rows = []  # (balance_id, inventory_kind, balance_date, warehouse_id, warehouse_code, product_code, product_barcode, product_id, card_code, expiry_date, serial_number, batch_number, quantity, measure_code, input_price, filial_id, filial_code)
    group_rows = []  # (balance_id, group_code, type_code)
    ...
    for entry in filial_warehouse_list:
        ...
        scope_key = make_scope_key(filial_id, warehouse_id)
        ...
        scope_added_f, scope_added_g = 0, 0
        ...
        for start, finish in daterange(effective_begin, effective_end, step_days=30):
            ...
            try:
                ...
                print(f"✅ {start.strftime(DATE_FORMAT)} - {finish.strftime(DATE_FORMAT)} | "
                      f"{scope_key} | items:{len(balance)} → +Fact:{added_f}, +Group:{added_g}")
                ...
                print(
                    f"⚠️ API xatosi | {scope_key} | {start.strftime(DATE_FORMAT)} - {finish.strftime(DATE_FORMAT)} | {e}")

        if scope_max_balance_date:
            upsert_scope_state(cursor, scope_key, scope_max_balance_date, scope_added_f)

    print(f"Σ API items: {total_items} | fact_rows:{len(fact_rows)} | group_rows:{len(group_rows)}")
...
def main():
    ...
    # 4) API dan ma’lumotlarni yig‘amiz (INCREMENTAL, per-scope)
    fact_rows, group_rows = fetch_balance_chunks(cursor, filial_warehouse_list, begin_date, end_date)
...
""")
    ...
    # 6) Bulk insert (Unicode safe) — xatoda fallback + temp table recreate
    try:
        ...
        # Ba'zi drayverlarda xato bo'lsa batch abort → temp jadvallar "yo'q" bo‘lib qoladi
        create_temp_tables()
